...
$page_is_all = false;
if (isset($page_type) && 'all' === $page_type) {
    $page_is_all = true;
        ...
        <?php foreach ($productsData['products'] as $product) { ?>
            <?php if ($product['category'] === $currentCategory) { ?>
                <div
                    ...
                    <a class="card card-compact bg-base-300 w-fit rounded-b-none h-full"
                       href="/products?id=<?php echo $product['id']; ?>">
                        <figure>
                            <img src="<?php echo $product['thumbnail']; ?>" alt="<?php echo $product['title']; ?>" class="aspect-video object-cover" />
                        </figure>
                        <div class="card-body">
